...
class Rba(Policy):
    ...
    def __init__(self, view, controller):
        super(Rba, self).__init__(view, controller)
    ...
    def sort_vnfs_by_cpu(sfc):
        ...
        dict_vnfs = {}
        for vnf in sfc:
            dict_vnfs[vnf] = vnfs_cpu.get(vnf, 0)
        sorted_dict_vnfs = dict(sorted(dict_vnfs.items(), key=lambda item: item[1], reverse=True))
        vnfs_sorted_asc_by_cpu = list(sorted_dict_vnfs.keys())
        return vnfs_sorted_asc_by_cpu


    def get_nfv_nodes_min_cpu(self, path):
        dict_nfv_nodes_min_cpu = {}
        for node in path:
            if self.view.is_nfv_node(node):
                dict_nfv_nodes_min_cpu[node] = self.controller.sum_vnfs_cpu_on_node(node)
        sorted_dict_nfv_nodes_min_cpu = dict(sorted(dict_nfv_nodes_min_cpu.items(), key=lambda item: item[1]))
        nfv_nodes_sorted_asc_by_cpu = list(sorted_dict_nfv_nodes_min_cpu.keys())
        return nfv_nodes_sorted_asc_by_cpu



    def place_vnfs_rba_algo(self, path, sfc):
        vnfs_sorted_asc_by_cpu = Rba.sort_vnfs_by_cpu(sfc)
        nfv_nodes_sorted_asc_by_cpu = self.get_nfv_nodes_min_cpu(path)
        for nfv_node, vnf in zip(nfv_nodes_sorted_asc_by_cpu, vnfs_sorted_asc_by_cpu):
            self.controller.put_vnf(nfv_node, vnf)
    ...
    def process_event(self, time, sfc_id, ingress_node, egress_node, sfc, delay, log):
        delay_sfc = {}  # dict to store the delay taken to run the sfc over the path
        path = self.view.shortest_path(ingress_node, egress_node)
        self.place_vnfs_rba_algo(path, sfc)
        self.controller.start_session(time, sfc_id, ingress_node, egress_node, sfc, delay, log)
        vnf_status = {vnf: 0 for vnf in sfc}  # 0 - not processed / 1 - processed
        sum_cpu_sfc = Hod.sum_vnfs_cpu(sfc)  # total time processing of the sfc
        delay_sfc[sfc_id] = 0
...
class Cba(Policy):
    ...
    def __init__(self, view, controller):
        super(Cba, self).__init__(view, controller)
        topology = view.topology()
        self.dict_nodes_deg = nx.degree_centrality(topology)
    ...
    def get_nodes_degree_path(self, path, n_nodes):
        dict_nodes_path = {node: deg for node, deg in  self.dict_nodes_deg.items() if node in path}
        top_nodes_high_deg = heapq.nlargest(n_nodes, dict_nodes_path, key=dict_nodes_path.get)
        return top_nodes_high_deg
    ...
    def place_vnfs_cba_algo(self, path, sfc):
        dict_deg_path = {}
        for node in path:
            dict_deg_path[node] = self.dict_nodes_deg.get(node, 0)
        sorted_dict_deg_path = dict(sorted(dict_deg_path.items(), key=lambda item: item[1], reverse=True))
        nodes_high_deg_path = list(sorted_dict_deg_path.keys())
        vnfs_sorted_asc_by_cpu = Cba.sort_vnfs_by_cpu(sfc)
        nfv_nodes_sorted_asc_by_cpu = self.get_nfv_nodes_min_cpu(nodes_high_deg_path)
        n_nodes = len(nfv_nodes_sorted_asc_by_cpu)
        top_nodes_high_deg = self.get_nodes_degree_path(path, n_nodes)
        for nfv_node, vnf in zip(top_nodes_high_deg, vnfs_sorted_asc_by_cpu):
            self.controller.put_vnf(nfv_node, vnf)
    ...
    def process_event(self, time, sfc_id, ingress_node, egress_node, sfc, delay, log):
        delay_sfc = {}  # dict to store the delay taken to run the sfc over the path
        path = self.view.shortest_path(ingress_node, egress_node)
        self.place_vnfs_cba_algo(path, sfc)
        self.controller.start_session(time, sfc_id, ingress_node, egress_node, sfc, delay, log)
        vnf_status = {vnf: 0 for vnf in sfc}  # 0 - not processed / 1 - processed
        sum_cpu_sfc = Hod.sum_vnfs_cpu(sfc)  # total time processing of the sfc
        delay_sfc[sfc_id] = 0
