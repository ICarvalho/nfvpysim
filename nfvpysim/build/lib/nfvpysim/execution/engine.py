...
def exec_experiment(topology, workload, netconf, policy, nfv_cache_policy, collectors):
    ...
    model_cba = NetworkModelCba(topology, nfv_cache_policy, **netconf)
    ...
    view_cba = NetworkViewCba(model_cba)
    ...
    controller_cba = NetworkController(model_cba)
    ...
    if policy['name'] == 'CBA':
        collectors_inst_cba = [DATA_COLLECTOR[name](view_rba, **params)
                                     for name, params in collectors.items()]
        collector_cba = CollectorProxy(view_rba, collectors_inst_cba)
        controller_cba.attach_collector(collector_cba)

        policy_name_cba = policy['name']
        policy_args_cba = {k: v for k, v in policy.items() if k != 'name'}
        policy_inst_cba = POLICY[policy_name_cba](view_cba, controller_cba, **policy_args_cba)

        for time, event in workload:
            policy_inst_cba.process_event(time, **event)
        return collector_cba.results()
